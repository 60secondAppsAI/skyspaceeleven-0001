					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateBookingAgent()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2BookingAgents = true">Add</base-button>
								              <modal :show.sync="modal2BookingAgents">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Booking Agent Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="BookingAgentId" type="text" placeholder="Enter BookingAgentId" v-model="bookingAgentDetails.bookingAgentId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Name</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Name" type="text" placeholder="Enter Name" v-model="bookingAgentDetails.name"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Agency Name</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="AgencyName" type="text" placeholder="Enter AgencyName" v-model="bookingAgentDetails.agencyName"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Contact Email</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="ContactEmail" type="text" placeholder="Enter ContactEmail" v-model="bookingAgentDetails.contactEmail"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  data() {
    return {
      bookingAgentToAdd: {},
      modal2BookingAgents: false,
      bookingAgentDetails: null,
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2BookingAgents = false;
    ...
    async updateBookingAgent() {

      const jsonData = JSON.stringify(this.bookingAgentDetails);
      const res = await BookingAgentService.update(jsonData);
...
//        this.getAllBookingAgents();
    ...
    async getBookingAgentDetails() {
      try {
			let response = await BookingAgentService.get(this.bookingAgentId);
			this.bookingAgentDetails = response.data;
