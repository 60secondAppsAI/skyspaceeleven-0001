        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalScheduledEvents = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalScheduledEvents">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add ScheduledEvent</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ScheduledEventId" type="text" placeholder="Enter ScheduledEventId" v-model="scheduledEventToAdd.scheduledEventId"></base-input>
  <base-input label="EventTime" type="text" placeholder="Enter EventTime" v-model="scheduledEventToAdd.eventTime"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="scheduledEventToAdd.description"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="scheduledEvents" :row-key="record => record.ScheduledEventId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ScheduledEventPictureView :scheduledEvents="scheduledEvents" />
          </div>
...
import ScheduledEventPictureView from './ScheduledEventPictureView.vue';


const scheduledEventsColumns = [
  "scheduledEventId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    ScheduledEventPictureView
  },

  data() {
    return {
      modalScheduledEvents: false,
        isTableView: true,

      columns: [
        {
          title: 'Scheduled Event Id',
		dataIndex: 'scheduledEventId',
          visible: true,
          scopedSlots: { customRender: 'scheduledEventId' },
          sorter: true
          //sorter: (a, b) => a.scheduledEventId - b.scheduledEventId,
          //sorter: (a, b) => a.scheduledEventId.localeCompare(b.scheduledEventId),
        },
        {
          title: 'Event Time',
		dataIndex: 'eventTime',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} scheduledEvents`,
      },
      ...
      sortBy: 'scheduledEventId',           // Column to sort by
  ...
  methods: {
    ...
    async renderScheduledEventsTable() {
      ...
      let scheduledEventsTableData = [];
      for (let i = 0; i < this.scheduledEvents.length; i++) {
        scheduledEventsTableData.push({
          id: i,
          scheduledEventId: this.scheduledEvents[i].scheduledEventId,
          year: this.scheduledEvents[i].year,
          date: this.scheduledEvents[i].date,
          competitionId: this.scheduledEvents[i].competitionId,
          scheduledEventId: this.scheduledEvents[i].scheduledEventId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-scheduled-events',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForScheduledEventsChanged', this.searchQuery);
		//this.renderScheduledEventsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalScheduledEvents = false;

      const currentDate = new Date().getTime();
      this.scheduledEventToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.scheduledEventToAdd);
      console.log(jsonData);
      
      const res = await ScheduledEventService.addScheduledEvent(jsonData);
