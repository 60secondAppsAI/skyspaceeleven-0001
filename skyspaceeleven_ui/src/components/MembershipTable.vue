        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalMemberships = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalMemberships">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Membership</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="MembershipId" type="text" placeholder="Enter MembershipId" v-model="membershipToAdd.membershipId"></base-input>
  <base-input label="Points" type="text" placeholder="Enter Points" v-model="membershipToAdd.points"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="memberships" :row-key="record => record.MembershipId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <MembershipPictureView :memberships="memberships" />
          </div>
...
import MembershipPictureView from './MembershipPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    MembershipPictureView
  },

  data() {
    return {
      modalMemberships: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'membershipId',
          visible: true,
          scopedSlots: { customRender: 'membershipId' },
          sorter: true
          //sorter: (a, b) => a.membershipId - b.membershipId,
          //sorter: (a, b) => a.membershipId.localeCompare(b.membershipId),
        },
        {
          title: 'Points',
		dataIndex: 'points',
      ...
      sortBy: 'membershipId',           // Column to sort by
  ...
  methods: {
    ...
    async renderMembershipsTable() {
      ...
      let membershipsTableData = [];
      for (let i = 0; i < this.memberships.length; i++) {
        membershipsTableData.push({
          id: i,
          membershipId: this.memberships[i].membershipId,
          year: this.memberships[i].year,
          date: this.memberships[i].date,
          competitionId: this.memberships[i].competitionId,
          membershipId: this.memberships[i].membershipId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-memberships',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForMembershipsChanged', this.searchQuery);
		//this.renderMembershipsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalMemberships = false;

      const currentDate = new Date().getTime();
      this.membershipToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.membershipToAdd);
      console.log(jsonData);
      
      const res = await MembershipService.addMembership(jsonData);
