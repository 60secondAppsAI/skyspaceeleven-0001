...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalServices = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalServices">
                <template slot="header">
  ...
  <base-input label="Price" type="text" placeholder="Enter Price" v-model="serviceToAdd.price"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="services" :row-key="record => record.ServiceId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ServicePictureView :services="services" />
          </div>
...
import ServicePictureView from './ServicePictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      sortBy: 'serviceId',           // Column to sort by
  ...
  methods: {
    ...
    async renderServicesTable() {
      ...
      let servicesTableData = [];
      for (let i = 0; i < this.services.length; i++) {
        servicesTableData.push({
          id: i,
          serviceId: this.services[i].serviceId,
          year: this.services[i].year,
          date: this.services[i].date,
          competitionId: this.services[i].competitionId,
          serviceId: this.services[i].serviceId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-services',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForServicesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.serviceToAdd.created = currentDate;
