        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Flight Crews</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalFlightCrews = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalFlightCrews">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add FlightCrew</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="FlightCrewId" type="text" placeholder="Enter FlightCrewId" v-model="flightCrewToAdd.flightCrewId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="flightCrews" :row-key="record => record.FlightCrewId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <FlightCrewPictureView :flightCrews="flightCrews" />
          </div>
...
import FlightCrewPictureView from './FlightCrewPictureView.vue';


const flightCrewsColumns = [
  "flightCrewId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    FlightCrewPictureView
  },

  data() {
    return {
      modalFlightCrews: false,
        isTableView: true,

      columns: [
        {
          title: 'Flight Crew Id',
		dataIndex: 'flightCrewId',
          visible: true,
          scopedSlots: { customRender: 'flightCrewId' },
          sorter: true
          //sorter: (a, b) => a.flightCrewId - b.flightCrewId,
          //sorter: (a, b) => a.flightCrewId.localeCompare(b.flightCrewId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} flightCrews`,
      },
      ...
      flightCrewsTable: {
        columns: [...flightCrewsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'flightCrewId',           // Column to sort by
  ...
  methods: {
    ...
    async renderFlightCrewsTable() {
      ...
      let flightCrewsTableData = [];
      for (let i = 0; i < this.flightCrews.length; i++) {
        flightCrewsTableData.push({
          id: i,
          flightCrewId: this.flightCrews[i].flightCrewId,
          year: this.flightCrews[i].year,
          date: this.flightCrews[i].date,
          competitionId: this.flightCrews[i].competitionId,
          flightCrewId: this.flightCrews[i].flightCrewId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-flight-crews',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForFlightCrewsChanged', this.searchQuery);
		//this.renderFlightCrewsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalFlightCrews = false;

      const currentDate = new Date().getTime();
      this.flightCrewToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.flightCrewToAdd);
      console.log(jsonData);
      
      const res = await FlightCrewService.addFlightCrew(jsonData);
