        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Flight Insurances</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalFlightInsurances = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalFlightInsurances">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add FlightInsurance</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="FlightInsuranceId" type="text" placeholder="Enter FlightInsuranceId" v-model="flightInsuranceToAdd.flightInsuranceId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="flightInsurances" :row-key="record => record.FlightInsuranceId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <FlightInsurancePictureView :flightInsurances="flightInsurances" />
          </div>
...
import FlightInsurancePictureView from './FlightInsurancePictureView.vue';


const flightInsurancesColumns = [
  "flightInsuranceId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    FlightInsurancePictureView
  },

  data() {
    return {
      modalFlightInsurances: false,
        isTableView: true,

      columns: [
        {
          title: 'Flight Insurance Id',
		dataIndex: 'flightInsuranceId',
          visible: true,
          scopedSlots: { customRender: 'flightInsuranceId' },
          sorter: true
          //sorter: (a, b) => a.flightInsuranceId - b.flightInsuranceId,
          //sorter: (a, b) => a.flightInsuranceId.localeCompare(b.flightInsuranceId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} flightInsurances`,
      },

      flightInsurances: [],
      flightInsuranceToAdd: {},

      flightInsurancesTable: {
        columns: [...flightInsurancesColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'flightInsuranceId',           // Column to sort by
  ...
  methods: {
    ...
    async renderFlightInsurancesTable() {
      ...
      let flightInsurancesTableData = [];
      for (let i = 0; i < this.flightInsurances.length; i++) {
        flightInsurancesTableData.push({
          id: i,
          flightInsuranceId: this.flightInsurances[i].flightInsuranceId,
          year: this.flightInsurances[i].year,
          date: this.flightInsurances[i].date,
          competitionId: this.flightInsurances[i].competitionId,
          flightInsuranceId: this.flightInsurances[i].flightInsuranceId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-flight-insurances',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForFlightInsurancesChanged', this.searchQuery);
		//this.renderFlightInsurancesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalFlightInsurances = false;

      const currentDate = new Date().getTime();
      this.flightInsuranceToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.flightInsuranceToAdd);
      console.log(jsonData);
      
      const res = await FlightInsuranceService.addFlightInsurance(jsonData);
